...
F.sub2use               = [1:17 19:22];%:53; # mismatch between behavioral and eeg data due to stopping experiment in between two blocks --> more trials in eeg than on behavior

F.trigger               = {[201] [202]}; % {'target primed';'target nonprimed';'distractor'}
F.trigger_cue           = {[11 12 16 17 18 19]; ...   %RDK1  attended conventional flicker [t; d; t+t; t+d; d+t; d+d]
...
for i_sub = 1:numel(F.sub2use)
    ...
    % %%%% this doesn't work, needs trouble shooting !!start
    ...
    t.behavior = horzcat(behavior.resp.experiment{cellfun(@(x) isstruct(x), behavior.resp.experiment)});
    % index trials with events (as only those were selected previously)
    t.trialwithbehv = [t.behavior([t.behavior.eventnum]>0).trialnumber];
    prep_input.PreProc.trial_orignrtrialinexp = t.trialwithbehv(prep_input.PreProc.trial_nrtrialinexp);
    t.prep_idx = prep_input.PreProc.trial_orignrtrialinexp( ...
        prep_input.PreProc.trial_blink & prep_input.PreProc.trial_eyemov & prep_input.PreProc.trial_SCADS);
    t.prep_idx2 = find( ...
        prep_input.PreProc.trial_blink & prep_input.PreProc.trial_eyemov & prep_input.PreProc.trial_SCADS);
    t.prep_idx3 = prep_input.PreProc.trial_nrtrialinexp( ...
        prep_input.PreProc.trial_blink & prep_input.PreProc.trial_eyemov & prep_input.PreProc.trial_SCADS);
    t.behavior = t.behavior(t.prep_idx);
    ...
    t.ur_epoch2 = num2cell(t.prep_idx3);
    [t.behavior.urepoch] = deal(t.ur_epoch{:});
    [t.behavior.urepoch2] = deal(t.ur_epoch2{:});
    ...
    for i_event = 1:numel(t.behavior)
        ...
        % condition
        t.behavior(i_event).condition_label = F.conname_2_withinlabel{t.behavior(i_event).condition};
        % event RDK
        t.behavior(i_event).eventRDK = t.behavior(i_event).eventRDK(t.idx);
        % event type
        t.behavior(i_event).eventtype = F.conname_1_withinlabel{t.behavior(i_event).eventtype(t.idx)};
        % event color
