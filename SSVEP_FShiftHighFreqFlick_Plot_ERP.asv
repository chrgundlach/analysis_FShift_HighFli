%% plot TFA images
clearvars
F.PathInEEG             = '\\smbone.dom.uni-leipzig.de\FFL\AllgPsy\experimental_data\2025_FShiftHiFli\eeg\erp'; 
...
F.Subs2use              = [1:17 19:22];%:53; # mismatch between behavioral and eeg data due to stopping experiment in between two blocks --> more trials in eeg than on behavior


F.conname_1_within        = 'event_type';
F.conname_1_withinlabel   = {'target';'distractor'};
F.conname_2_within        = 'condition';
F.conname_2_withinlabel   = {'conventional';'conventional';'highfreq_col';'highfreq_col';'highfreq_white';'highfreq_white'};
F.conname_3_within        = 'attention';
F.conname_3_withinlabel   = {'att RDK1';'att RDK2'; 'att RDK1';'att RDK2';'att RDK1';'att RDK2'};
%% load data
...
pl.con_contrast = {... % contrasts by 1st dim; averaged across second dim
    'condition_label', {{'conventional'};{'highfreq_col'};{'highfreq_white'}}; ...
...
% reshape pl.data
pl.dat2plot_rs = mean(squeeze(pl.dat2plot),ndims(pl.dat2plot));
if ndims(pl.dat2plot_rs)==3
    pl.con_label = [pl.con_contrast{1,2}{:}];
    plot_data_topoarray(EP.electrodes, pl.dat2plot_rs,'ERP','times',EP.time,'conds',pl.con_label)
elseif ndims(pl.dat2plot_rs)==4
    for i_plot = 1:numel(pl.con_contrast{2,2})
        pl.con_label = [pl.con_contrast{1,2}{:}];
        plot_data_topoarray(EP.electrodes, pl.dat2plot_rs(:,:,:,i_plot), ...
            'ERP','times',EP.time,'conds',pl.con_label,'title',pl.con_contrast{2,2}{i_plot}{:})
    end

    for i_plot = 1:numel(pl.con_contrast{1,2})
        pl.con_label = [pl.con_contrast{2,2}{:}];
        plot_data_topoarray(EP.electrodes, squeeze(pl.dat2plot_rs(:,:,i_plot,:)), ...
            'ERP','times',EP.time,'conds',pl.con_label,'title',pl.con_contrast{1,2}{i_plot}{:})
    end
end


%% plot ERPs exploratively for specified electrodes | event type for condition specified first

% loop through conditions defined in contrasts
pl.con_contrast = {... % contrasts by 2nd dim; averaged across second dim
    %'trial_timing_type', {{'regular'}}; ...
    % 'condition_label', {{'conventional'}}; ...
    % 'condition_label', {{'highfreq_col'}}; ...
    'condition_label', {{'highfreq_white'}}; ...
    % 'condition_label', {{'conventional'};{'highfreq_white'}; {'highfreq_col'}}; ...
    % 'event_response_type', {{'hit','FA_proper','CR','miss'}}; ...
%     'evnt_type_label', {{'chroma+'}}; ...
%     'event_response_type', {{'hit','FA','error','miss'}}; ...
    'eventtype', {{'target'};{'distractor'}}};
    % 'eventtype2', {{'target primed'};{'target nonprimed'}}};
pl.sub2plot = 1:numel(F.Subs2use);
...
% pl.elec2plot = {'P10';'P9'}; % for P1 component lateral
% pl.elec2plot = {'POz';'Oz'}; % for P1 component lateral
...
pl.concols = num2cell(hex2rgb({'#006651';'#943826'})',1);
pl.concols = num2cell(hex2rgb({'#1E90FF';'#FF0000'})',1);
...
% reshape pl.data
if ndims(pl.dat2plot)>4
    pl.dat2plot_rs = squeeze(mean(pl.dat2plot,[1,3]));
else
...
title(sprintf('N=%1.0f\n%s = M(%s)',numel(pl.sub2plot), pl.con_contrast{1,1},vararg2str(pl.con_contrast{1,2}(:))), "Interpreter","none")
legend([h.plm{:}],[pl.con_contrast{end,2}{:}],'Location','EastOutside','box','off')
grid on
...
%% plot ERPs exploratively for specified electrodes | event type difference separately for for specified conditions
...
% reshape pl.data
pl.dat2plot_rs = squeeze(diff(mean(pl.dat2plot,[1]),1,4));
...
if ~isempty(pl.ylim)
    ...
    ylim(max(abs(get(gca,"YLim")))*[-1 1])
end
xlabel('time in ms')
ylabel('amplitude in \muV/cm²')
title(sprintf('N=%1.0f\nDiff_%s(%s-%s)',numel(pl.sub2plot), pl.con_contrast{2,1},pl.con_contrast{2,2}{1}{:},pl.con_contrast{2,2}{2}{:}), "Interpreter","none")
legend([h.plm{:}],[pl.con_contrast{1,2}{:}],'Location','EastOutside','box','off',"Interpreter","none")
grid on
set(gca, 'Ydir','reverse')
...
% loop through conditions defined in contrasts
pl.con_contrast = {... % contrasts by 2nd dim; averaged across first dim
    'eventtype2', {{'target primed'};{'target nonprimed'}}; ...
%     'evnt_type_label', {{'chroma+'}}; ...
...
%     'event_response_type', {{'hit','FA','error','miss'}}; ...
    'eventtype', {{'target'};{'distractor'}}};
    % 'eventtype2', {{'target primed'};{'target nonprimed'}}};
% pl.sub2plot = 1:numel(F.Subs2use);pl.sub2plot = 1:numel(F.Subs2use);

pl.time2plot = [200 300]; % time in ms
pl.time2plot = [250 350]; % time in ms
pl.time2plot = [400 600]; % time in ms
pl.time2plot = [100 200]; % time in ms
...
annotation('textbox',[.1 .3 .1 .2], ...
    'String',pl.con_contrast{1,2}{:}{:}, ...
    'EdgeColor','none', 'Rotation',90, 'Interpreter','none','FontWeight','bold')
